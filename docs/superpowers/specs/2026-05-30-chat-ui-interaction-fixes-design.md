...
| 4 | 工具卡片上下左右边距过大 | 各卡片 padding 值统一偏大 | 低 |
| 5 | 统计信息逐条显示，应在 agent 整次回答结束后只在最后一条汇总 | AssistantMessageCard 对每条 Assistant 消息独立聚合 stepFinishes 显示 Footer | 中 |
...
**文件**: `chat/markdown/MarkdownContent.kt`
...
当前 📋 按钮只出现在 `!isContinuation` 的 header 区域。设计 D 修改后，每条消息的 Footer 区域都有 📋 按钮。具体位置见设计 D。
...
5. **间距**: 视觉检查工具卡片展开/折叠态的间距
   - 验证：padding 和 spacing 约为原来的一半

### 单元测试补充

1. `ChatModifiersTest`: 验证 `consumeBoundaryScroll` 的 `onPostScroll` 边界消费逻辑
2. `TurnGroupTest`: 验证 turn 分组计算的正确性（单条、多条、混合 user/assistant）
3. `AssistantMessageCardTest`: 验证 `isTurnLast` 控制 Footer 显示的逻辑
