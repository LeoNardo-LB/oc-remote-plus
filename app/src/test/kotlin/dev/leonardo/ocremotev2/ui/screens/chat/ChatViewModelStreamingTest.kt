...
class ChatViewModelStreamingTest {
    ...
    fun setup() {
        ...
        chatRepository = mockk<ChatRepository>(relaxed = true).also {
            ...
            every { it.getParts(any()) } answers {
                ...
                partsFlow.map { map: Map<String, List<dev.leonardo.ocremotev2.domain.model.Part>> ->
                    map[sid] ?: emptyList()
                }
            }
    ...
    private fun stubUserMessage(id: String = "msg-1") {
        ...
        val textPart = dev.leonardo.ocremotev2.domain.model.Part.Text(
            id = "$id-text",
            sessionId = "test-session",
            messageId = id,
            text = "hello"
        ...
        coEvery { manageSessionUseCase.listMessages(any(), any(), any()) } returns listOf(
            dev.leonardo.ocremotev2.domain.model.MessageWithParts(info = userMsg, parts = listOf(textPart))
        )
    }
