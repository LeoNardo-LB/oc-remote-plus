...
/** 单个 item 的描述：key 与 contentType。Composable 内容由 AnchoredItemProvider 通过 composables 列表提供。 */
 ...
 * 只收集 key + contentType（不存储 composable lambda）——避免闪屏。
 * Composable 内容由 AnchoredLazyColumn 通过 itemContent 参数统一提供。
 */
class AnchoredLazyListScope {
    internal val items = mutableListOf<AnchoredItem>()
    internal val composables = mutableListOf<@Composable () -> Unit>()
    ...
    fun <T> itemsIndexed(
        ...
        list.forEachIndexed { index, item ->
            ...
            composables.add { itemContent(index, item) }
        }
    }
}
...
fun AnchoredLazyColumn(
    ...
    // 1. 每次 recompose 重新收集 items + composables。
    val scope = AnchoredLazyListScope().apply(content)

    // 2. composables 通过 rememberUpdatedState 跟踪最新值。
    //    provider 实例 stable → LazyLayoutItemContentFactory cache 保留
    //    → slot 复用 → ChatMessage equals 判断 → 只有变化的 item recompose → 无闪屏。
    val currentComposables by rememberUpdatedState(scope.composables)

    // 3. itemProvider stable 实例。
    val itemProvider = remember { AnchoredItemProvider { currentComposables } }
...
private class AnchoredItemProvider(
    ...
    private var _items by mutableStateOf<List<AnchoredItem>>(emptyList())

    fun updateItems(newItems: List<AnchoredItem>) {
        _items = newItems
    }

    override val itemCount: Int get() = _items.size

    override fun getContentType(index: Int): Any? = _items[index].contentType

    override fun getKey(index: Int): Any = _items[index].key
...
private fun LazyLayoutMeasureScope.measureAnchoredItems(
    ...
    // === 8.5. Scroll anchoring ===
    // 只在用户不在底部时（canScrollBackward=true）补偿 item[0] 的增长。
    // item[0] 是正在流式输出的消息（reverseLayout 下在视觉底部）。
    // 用 canScrollBackward 而非 isAtBottom 参数：后者在 item[0] 高度 > 视窗时误判。
    if (state.canScrollBackward && !state.isScrollInProgress && visibleItems.isNotEmpty()) {
        val item0 = visibleItems.find { it.index == 0 }
        if (item0 != null) {
            val prevSize = state.prevItemSizes[item0.key]
            if (prevSize != null && item0.height > prevSize + 1) {
                currentFirstItemScrollOffset += item0.height - prevSize
            }
        }
    }
